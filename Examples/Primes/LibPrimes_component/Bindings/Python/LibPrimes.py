...
'''
ProgressCallback = ctypes.CFUNCTYPE(ctypes.c_void_p, ctypes.c_float, ProgressIdentifier, ctypes.POINTER(ctypes.c_bool))
...
class LibPrimesWrapper:
	...
	def _loadFunctionTable(self):
		try:
			...
			self.lib.libprimes_calculator_getself.restype = ctypes.c_int64
			self.lib.libprimes_calculator_getself.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_void_p)]
...
class Calculator(Base):
	...
	def GetSelf(self):
		...
		self._wrapper.checkError(self, self._wrapper.lib.libprimes_calculator_getself(self._handle, ValueHandle))
		ValueObject = Calculator(ValueHandle, self._wrapper)
		return ValueObject
