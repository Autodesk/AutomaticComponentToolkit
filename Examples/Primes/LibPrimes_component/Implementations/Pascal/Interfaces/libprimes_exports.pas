...
* @return error code or 0 (success)
*)
function libprimes_factorizationcalculator_checkprimefactors (pFactorizationCalculator: TLibPrimesHandle; nPrimeFactorsCount: QWord; pPrimeFactorsBuffer: PLibPrimesPrimeFactor; pAreEqual: PCardinal): TLibPrimesResult; cdecl;
...
function libprimes_factorizationcalculator_checkprimefactors (pFactorizationCalculator: TLibPrimesHandle; nPrimeFactorsCount: QWord; pPrimeFactorsBuffer: PLibPrimesPrimeFactor; pAreEqual: PCardinal): TLibPrimesResult; cdecl;
var
	ResultAreEqual: Boolean;
	ObjectFactorizationCalculator: TObject;
	IntfFactorizationCalculator: ILibPrimesFactorizationCalculator;
begin
	try
		if ((not Assigned (pPrimeFactorsBuffer)) and (nPrimeFactorsCount>0)) then
			raise ELibPrimesException.Create (LIBPRIMES_ERROR_INVALIDPARAM);
		if not Assigned (pAreEqual) then
			raise ELibPrimesException.Create (LIBPRIMES_ERROR_INVALIDPARAM);
		if not Assigned (pFactorizationCalculator) then
			raise ELibPrimesException.Create (LIBPRIMES_ERROR_INVALIDPARAM);
		...
		if Supports (ObjectFactorizationCalculator, ILibPrimesFactorizationCalculator) then begin
			IntfFactorizationCalculator := ObjectFactorizationCalculator as ILibPrimesFactorizationCalculator;
			ResultAreEqual := IntfFactorizationCalculator.CheckPrimeFactors(nPrimeFactorsCount, pPrimeFactorsBuffer);

			pAreEqual^ := Ord (ResultAreEqual);
		end else
			raise ELibPrimesException.Create (LIBPRIMES_ERROR_INVALIDCAST);
