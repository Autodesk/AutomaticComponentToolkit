...
*)
function calculation_base_classtypeid(pBase: TCalculationHandle; pClassTypeId: PQWord): TCalculationResult; cdecl;
...
* @return error code or 0 (success)
*)
function calculation_calculator_enlistvariable(pCalculator: TCalculationHandle; pVariable: TNumbersHandle): TCalculationResult; cdecl;
...
* @return error code or 0 (success)
*)
function calculation_calculator_getenlistedvariable(pCalculator: TCalculationHandle; nIndex: Cardinal; pVariable: PNumbersHandle): TCalculationResult; cdecl;
...
* @return error code or 0 (success)
*)
function calculation_calculator_multiply(pCalculator: TCalculationHandle; pInstance: PNumbersHandle): TCalculationResult; cdecl;
...
* @return error code or 0 (success)
*)
function calculation_calculator_add(pCalculator: TCalculationHandle; pInstance: PNumbersHandle): TCalculationResult; cdecl;
...
begin
  try
    ...
    if Supports(ObjectBase, ICalculationBase) then begin
      IntfBase := ObjectBase as ICalculationBase;
    ...
    On E: ECalculationException do begin
      Result := HandleCalculationException(ObjectBase , E);
    end;
...
begin
  try
    ...
    if not Assigned(pCalculator.Handle) then
      raise ECalculationException.Create(CALCULATION_ERROR_INVALIDPARAM);

    ObjectCalculator := TObject(pCalculator.Handle);
...
begin
  try
    ...
    if Assigned(ResultInstance) then
      pInstance^.ClassTypeId := (ResultInstance as ICalculationBase).ClassTypeId()
    else
...
begin
  ...
  if (pProcName = 'calculation_base_classtypeid') then
    ppProcAddress := @calculation_base_classtypeid
  else if (pProcName = 'calculation_calculator_enlistvariable') then
