...
private:
	...
	static void readMethodInto(CalculationSymbolLookupType pLookupMethod, std::string sFunctionName, void** pfnTarget) {
		CalculationResult eLookupError = (*pLookupMethod)(sFunctionName.c_str(), pfnTarget);
		if ( (eLookupError != CALCULATION_SUCCESS) || (pfnTarget == nullptr) )
			throw ECalculationException(CALCULATION_ERROR_COULDNOTFINDLIBRARYEXPORT, "");
	}
...
class CBase {
private:
	sCalculationFunctionTableBase* m_pFunctionTableBase;
	inline static std::map<CalculationSymbolLookupType, sCalculationFunctionTableBase>& MapFunctionTableBase()
	{
		static std::map<CalculationSymbolLookupType, sCalculationFunctionTableBase> sMapFunctionTableBase;
		return sMapFunctionTableBase;
	}
protected:
	/* Handle to Instance in library*/
	CalculationExtendedHandle m_pHandle;

	/* Checks for an Error code and raises Exceptions */
	virtual void CheckError(CalculationResult nResult)
	{
		if (nResult != 0) {
...
public:
	...
	*/
	CBase(CalculationExtendedHandle pHandle)
		: m_pHandle(pHandle)
	{
		CalculationSymbolLookupType pLookupFunction = m_pHandle.m_pfnSymbolLookupMethod;
	...
	*/
	CalculationExtendedHandle GetHandle()
	{
		return m_pHandle;
	}
...
private:
	sCalculationFunctionTableCalculator* m_pFunctionTableCalculator;
	inline static std::map<CalculationSymbolLookupType, sCalculationFunctionTableCalculator>& MapFunctionTableCalculator()
	{
		static std::map<CalculationSymbolLookupType, sCalculationFunctionTableCalculator> sMapFunctionTableCalculator;
		return sMapFunctionTableCalculator;
	}
public:
	/**
	* CCalculator::CCalculator - Constructor for class Calculator.
	*/
	CCalculator(CalculationExtendedHandle pHandle)
		: CBase(pHandle)
	{
		CalculationSymbolLookupType pLookupFunction = m_pHandle.m_pfnSymbolLookupMethod;
		auto iFunctionTable = MapFunctionTableCalculator().find(pLookupFunction);
		if (MapFunctionTableCalculator().end() == iFunctionTable) {
			...
			CWrapper::readMethodInto(pLookupFunction, "calculation_calculator_multiply", (void**)&(m_pFunctionTableCalculator->m_Calculator_Multiply));
			CWrapper::readMethodInto(pLookupFunction, "calculation_calculator_add", (void**)&(m_pFunctionTableCalculator->m_Calculator_Add));
		} else {
			m_pFunctionTableCalculator = &(iFunctionTable->second);
		}
	}
	...
	*/
	...
	inline CalculationResult CWrapper::loadWrapperTable(sCalculationDynamicWrapperTable * pWrapperTable, const char * pLibraryFileName)
	{
		...
		#ifdef _WIN32
		pWrapperTable->m_Base_ReleaseInstance = (PCalculationBase_ReleaseInstancePtr) GetProcAddress(hLibrary, "calculation_base_releaseinstance");
		#else // _WIN32
		pWrapperTable->m_Base_ReleaseInstance = (PCalculationBase_ReleaseInstancePtr) dlsym(hLibrary, "calculation_base_releaseinstance");
		dlerror();
		#endif // _WIN32
		...
		#ifdef _WIN32
		pWrapperTable->m_Base_AcquireInstance = (PCalculationBase_AcquireInstancePtr) GetProcAddress(hLibrary, "calculation_base_acquireinstance");
		#else // _WIN32
		pWrapperTable->m_Base_AcquireInstance = (PCalculationBase_AcquireInstancePtr) dlsym(hLibrary, "calculation_base_acquireinstance");
		dlerror();
		#endif // _WIN32
	...
	inline CalculationResult CWrapper::loadWrapperTableFromSymbolLookupMethod(sCalculationDynamicWrapperTable * pWrapperTable, void* pSymbolLookupMethod)
{
		...
		readMethodInto(pLookupFunction, "calculation_calculator_multiply", (void**)&(pWrapperTable->m_Calculator_Multiply));
		readMethodInto(pLookupFunction, "calculation_calculator_add", (void**)&(pWrapperTable->m_Calculator_Add));
