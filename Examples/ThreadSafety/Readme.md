...
This concludes the implementation of version `1.0.0` of the library.
...
# 4.4. Java
The folder `Examples\Java` contains a minimal java file `LibPrimes_Example.java` that makes
use of the Java bindings as well as a minimal build script to compile bindings and the example and executes the example.
The generated Java bindings requires Java 9 and uses JNA (Java Native Access) to load the library and functions. Therefore the build script loads
the latest jna.jar file and shows how to handle the classpath.

The autogenerated java class prints the current library version:
```java
import libprimes.*;
...
Running this java program will, again, print:
```shell
LibPrimes version: 1.2.0
735 =  3^1 * 5^1 * 7^2 
```

## 4.5 Use of an ACT component in existing code bases
